...
```{r}
boxplot(log(esp_ope_selection$lop_effectif)~ esp_ope_selection$esp_code_alternatif)
```



```{r}
summary(esp_ope_selection)
```

```{r}
esp_table <- table(esp_ope_selection$esp_code_alternatif)
esp_table
```
