...
# Chargement de la liste d'espèce de la liste rouge 2016 Bourgogne
...
tax_ref_aspe_test <- tax_ref_aspe %>%
...
esp_ope_selection_test <- esp_ope_selection %>% 
  left_join(tax_ref_aspe_test, by = "esp_code_alternatif")

```

## Liste d'espèces de BFC à partir de la base ASPE

```{r}

list_esp <- esp_ope_selection_test %>% 
  select(nom_vernaculaire, nom_latin)
...
tab_densite_occurrence <- tab_densite_occurrence %>%
  mutate(statut_lrr = ifelse (esp_code_alternatif %in% params$especes_lrr_2015, 
                           "Liste Rouge Régionale BFC 2016",
