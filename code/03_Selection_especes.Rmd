...
params:
  especes_lrr_2014: !r c("LOT", "LOE", CHE", "BRO","BOU", "ANG", "PER", "BAF", "EPT", "ABH, "BRE", "EPI", "CCO", "CHA", "HOT", "VAI", "TAN", "BLN", "VAN", "TRF", "LPP", "OBR", "ROT", "ABL", "GAR", "IDE", "LOF", "TOX", "GRE", "BRB", "SPI", "GOU", "LPM", "LOR", "LPP")
...
tax_ref_uicn <- base_tax_ref_uicn %>%
  ...
  filter(classe == "Actinopterygii" | classe == "Petromyzonti", fr == "P") %>%
  select(cd_nom, classe, ordre, famille, nom_complet, nom_vern) %>%
...
tax_ref_aspe <- tax_ref_aspe %>%
 ...
 # mutate(esp_code_alternatif = fct_recode(esp_code_alternatif, "CAA" = "CAD")) %>% # Correction de l'ancien code du Carassin doré de CAD en CAA
  select(esp_code_alternatif, nom_vernaculaire, nom_latin, nom_complet)
...
```{r Barplot effectif total espèce,  fig.height = 8, fig.width = 10}}
...
ggplot(esp_effectif,
  ...
  theme_minimal() +
  labs(x = "Espèce", y = "Effectif total") +
  theme(
...
```{r Barplot effectif total espèce log}
ggplot(esp_effectif,
       aes(x = esp_code_alternatif, y = effectif_total, fill = effectif_total)) +
  geom_col() +
  scale_fill_gradient(low = "darkseagreen3", high = "darkseagreen4") +
...
esp_liste_bfc <- esp_list %>% 
  ...
  select(nom_vernaculaire,nom_latin, nom_complet,esp_code_alternatif, Introduction, effectif_total)
```


## Liste d'espèces de BFC sélectionnées à partir de la base ASPE
...
tab_densite_occurrence <- tab_densite_occurrence %>%
  mutate(statut_lrr = ifelse (esp_code_alternatif %in% params$especes_bfc, 
                           "Liste Rouge Régionale BFC 2014",
...
```
  ...
  Dans le cadre de notre étude, on observe que les espèces étudiées dans la précédente liste rouge régionale (2015) apparaissent pour la quasi-totalité dans la partie supérieure droite du graphique, à l'exception de quelques espèces comme : 
