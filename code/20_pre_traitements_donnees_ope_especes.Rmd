...
subtitle: "20_pre_traitements_donnees_ope_especes"
...
#   db = NULL
# ) %>%
#   filter(!is.na(LengthFecunMin)) %>%
#   filter(LengthTypeFecMin == "TL")
# 
# 
# summary(asp$LengthFecunMin)
# 
# 
# van <- rfishbase::fecundity(
#   "Leuciscus leuciscus",
#   species_list = NULL,
#   fields = NULL,
#   server = c("fishbase"),
...
#   db = NULL
# ) %>%
#   filter(Locality == "France")
# 
# tox <- rfishbase::fecundity(
...
```{r jointure manuelle mei}
# mei_for_join <- temp_df %>%
#   rename(lop_id = mei_lop_id)

```

```{r renommer temp_df en mesure_individuelle}
mesure_individuelle <- temp_df # renommer temp_df en mesure_individuelle pour pouvoir utiliser correctement les fonctions du package Aspe 
```
...
# Modif provisoire : Pour le df final script GLMM et GAMMS
...
```{r Graphique poids réel/estimé, fig.height = 25, fig.width = 25}
...
On exporte le jeu de data pour corriger la base de data aspe 

```{r}
readr::write_excel_csv(erreurs_df, "../processed_data/error_data_aspe.csv")
```
...
```{r resume du df mei_ope_selection}
mei_ope_selection <- mei_ope_selection %>% 
