...
On observe dans les series temporelles qu'il y a des stations qui sont prospecter avec des protocoles différents, ce qui n'est pas censé être le cas. On détecte alors dans le jeu de données les stations avec des protocoles différents et les stocke dans un tableau *ope_error_protocol*: 
```{r detection stations avec erreur, plusieurs protocoles}
ope_error_protocol <- ope_selection %>%
  select(sta_id,pro_libelle, pop_libelle) %>% 
  group_by(sta_id) %>% 
  filter(n_distinct(pro_libelle) > 1) %>% 
  ungroup() %>% 
  count(pop_libelle, pro_libelle) %>% 
  pivot_wider(
    names_from = pro_libelle,
    values_from = n, 
    values_fill = 0
  )
```

```{r tableau stations avec erreur protocoles}
ope_error_protocol %>%
  flextable::flextable() %>%
  flextable::autofit() %>%
