...
Script : 01_pretreatment_ope
  ...
  L'objectif de cette étude est d'évaluer les tendances démographiques des poissons d'eau douce de de Bourgogne-Franche-comté. Ce travail vient en appui de la révision de la liste rouge régionale de 2017.
...
rdata_tables <- misc_nom_dernier_fichier(
  repertoire = "../../../../Liste_rouge_BFC/processed_data",
  pattern = "^tables")
...
# Sélection d’un périmètre géographique 
  ...
  Pour notre étude, nous nous concentrerons sur la Bourgogne-Franche-Comté, incluant les départements 89, 21, 58, 71, 70, 25 et 39. Ces départements sont modifiables selon la région et l'objectif de l'étude.
...
aspe_bfc <- passerelle %>% 
  mef_ajouter_dept() %>% 
  filter(dept == c( "21","25","39", "58", "70","71","89")) # Sélection de l'aire géographique : choix des départements 
```
...
buffer <- st_buffer(aspe_bfc, # Mise en place d'un buffer 
