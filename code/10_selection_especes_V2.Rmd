...
subtitle: "10_selection_especes_V2"
...
## Construction du jeu de donnée finale **esp_ope_selection** : A MODIFIER !!!!

Pour construire le df **esp_ope_selection**, pusieurs jeu de donnée vont être fusionné :  

1. **tax_ref_aspe** possède les codes espèces de la base aspe ainsi que les noms des espèces (taxref).

2. **liste_esp_bfc** possède les codes espèces, une liste des espèces de 2016 sur la région avec les statuts d'autochtonies.

3. **base_liste_complete_bfc_aspe** correspond au df de la période complète régionale de la base aspe toute stations confondues, export présent directement dans *processed_data*.

4. **liste_correc_autochtonie** correspond à une liste provisoire issus des fusions des df précédent. Il est présent en export dans *processed_data*

5. **liste_bfc_sp_cor** correspond à un df de toutes les espèces présentes en BFC avec les noms taxref, les codes aspe, les statut d'autochtonie par bassins- versants et quelques corrections. Il est présent en export dans *processed_data*. Il a été construit à partir de la fusion des jeux de données précédents ainsi qu'un ajout de correction manuellement sur de nouvelles colonnes, sur Excel. Il est une correction directe du df précédent **liste_correc_autochtonie**. 

6. **esp_ope_selection** correspond au df de la période *2007-2025* avec les stations sélectionnées pour l'analyse des données.
...
```{r ajout des noms des especes taxref}
tax_ref_aspe <- ref_espece %>% # Ajout du dataframe ref_espece de la base aspe contennant les noms des espèces
...
esp_full_list <- esp_list %>% 
  ...
  select(nom_vernaculaire, nom_latin,esp_nom_commun, esp_nom_latin, esp_code_alternatif, famille, introduction, effectif_total)
...
table_liste_bfc_sp_cor_exp <- liste_bfc_sp_cor %>%
  rename(
    ...
    "Nom vernaculaire" = esp_nom_commun
  ) %>%
  arrange(desc(Statut))
...
```{r tableau LR especes finale, fig.height = 8, fig.width = 10}
...
```
```{r}
tab_densite_occurrence$esp_code_alternatif
...
```
  "ABL"   "ANG"   "BAF"   "BLN"   "BRB"  "BRE"   "BRO"   "CCO"   "CHA"   "CHE"   "EPI"   "EPT"   "GAR"   "GOU"  "HOT"   "LOF"   "LOR"   "LOT"   "LPM"   "LPP"   "OBR"   "PER"   "ROT"   "SAT"   "SPI"   "TAN"   "TOX"   "TRF"   "VAI"   "VAN"   "VAR" 


```{r Filtrage espèce tab_occurence}
tab_occurrence_esp <- tab_occurrence %>% 
  filter(esp_code_alternatif%in% mes_especes)
```

```{r}
ope_esp_densite_cal <- ope_esp_densite_cal %>% 
  filter(esp_code_alternatif%in% mes_especes)
```
...
```{r}
traits_biologiques <- readxl::read_xlsx("../raw_data/traits_biologiques.xlsx")

traits_biologiques <- traits_biologiques %>%
  select(esp_code_alternatif, temps_generation, age_maturite)
...
```{r exporter la liste des temps de générations}
# write_excel_csv(liste_bio_join, "../processed_data/temps_de_generation_pro.csv")
```

# Sauvegarde 

Sauvegarde pour le script suivant : script 20_pre_traitements_donnees_ope_especes.
