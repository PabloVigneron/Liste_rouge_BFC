...
```{r, ajouter annee au df densite}
ope_esp_densite_cal <- ope_esp_densite_cal %>%
  left_join(esp_ope_selection, by = c("ope_id", "esp_code_alternatif")) %>%
  select(ope_id, esp_code_alternatif, annee, valeur)
```


```{r, fig.height = 20, fig.width = 20}
aspe::gg_temp_abondance(
  df = ope_esp_densite_cal ,
  var_espece = esp_code_alternatif,
  var_abondance = valeur,
  nb_colonnes = 6)
```
...
test <- esp_ope_selection %>% 
  ...
  left_join(liste_bfc_sp_cor, by = "esp_code_alternatif") %>%# Ajout libelle , noms d'espèces
  select(ope_id, lop_effectif, lop_id, esp_code_alternatif, annee, pop_id, nom_valide)
...
ggplot(data = sp_occur_by_year, aes(x = annee, y = n_occur)) +
  geom_point() +
