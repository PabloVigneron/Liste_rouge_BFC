...
reg_indicateur <- ope_indicateur %>%
  ...
  dplyr::summarize(valeur = if (unique(indicateur) == "pourcentage_juveniles") {
    #Calculer la moyenne pour le pourcentage de juvéniles
    mean(valeur, na.rm = TRUE)
  } else {
    median(valeur, na.rm = TRUE)  # Calculer la médiane pour les autres
  }, .groups = "drop") %>%
  distinct()
```
