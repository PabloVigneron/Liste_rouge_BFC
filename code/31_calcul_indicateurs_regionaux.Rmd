...
subtitle: "31_calcul_indicateurs_regionaux"
...
```{r}
# Compte le nombre total d'opérations distinctes pour chaque année par espèce
n_ope_par_an <- ope_indicateur %>%
  filter(stade == "ind",                        # Filtre les lignes où le stade ="ind"
         indicateur == "effectif_total") %>% # Filtre les lignes où l'indicateur ="effectif_total"
  group_by(annee, 
           esp_code_alternatif) %>%                       # Groupe par année
  summarise(n_ope = n_distinct(ope_id))     # Compte le nombre distinct d'ope_id et l'assigne à n_ope
```
...
reg_indicateur <- ope_indicateur %>%
  group_by(
    esp_code_alternatif,
    # Regrouper par code alternatif de l'espèce
    annee,
    # Regrouper par année
    indicateur,
    # Regrouper par type d'indicateur
    stade # Regrouper par stade
  ) %>%
  # Calculer la moyenne pour le pourcentage de juvéniles et la densité surfacique
  
  dplyr::summarize(valeur = case_when(
    first(indicateur) %in% c("pourcentage_juveniles", "densite_surface", "biomasse") ~ mean(valeur, na.rm = TRUE),
    TRUE ~ sum(valeur, na.rm = TRUE)# Calculer la médiane pour les autres
  ),
  .groups = "drop")
```
...
reg_taux_evol_2007 <- calcul_taux_evol(df = reg_indicateur,
                             var_x = annee,
                             var_y = valeur,
                             debut = 2007,
                             esp_code_alternatif,
                             stade,
                             indicateur)
print(reg_taux_evol_2007)
```

Nous avons donc, à la fin de ce script, un tableau de bord d'indicateurs simples à l'échelle régionale. Nous allons pouvoir analyser les différentes tendances des indicateurs sur la période 1990-2023 dans le prochain script (?_analyse_indicateurs_regionaux). 
