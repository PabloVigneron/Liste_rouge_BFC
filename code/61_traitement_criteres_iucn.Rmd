...
subtitle: "61_traitement_criteres_iucn"
author: "Pablo Vignerone"
...
Script : 30_application_criteres_lrr_uicn
...
```{r}
temps_generation_lrr_2014 <- readxl::read_xls("../raw_data/temps_generation_lrr_2014_BFC.xls", sheet = 3) %>% 
  janitor::clean_names()

temps_generation_lrr_2014 <- temps_generation_lrr_2014 %>% 
  select(
    esp_code_alternatif, 
    maturite_sexuelle_males, 
    maturite_sexuelle_femelles, 
    longevite_males,
    longevite_femelles,
    x3_generations, 
    x1_generation
...
```{r}
temps_generation_lrr_bzh <- readxl::read_xlsx("../processed_data/temps_de_generation_pro.xlsx")

```


```{r}
temps_genaration <- temps_generation_lrr_bzh %>% 
  left_join(temps_generation_lrr_2014 , by = "esp_code_alternatif") %>% 
  select(
    esp_code_alternatif, 
    periode_lrr,  
    x3_generations
...
```{r}
mes_especes <- c("ABL", "ANG", "BAF", "BLN", "BRB", "BRE", "BRO", "CCO", "CHA", "CHE", "EPI", "GAR", "GOU" , "LOF" ,"LOR", "LOT", "LPP", "OBR", "PER" ,"ROT", "SPI", "TAN", "TRF", "VAI", "VAN")
```
...
```{r}
# Correctiond des espèces qui finalement sont en DD !
df_zip_glmm_resultats_cor <- df_zip_glmm_resultats %>% 
  filter(esp_code_alternatif %in% mes_especes)
```
...
statut_pourcentage_evo <- df_zip_glmm_resultats_cor %>%
  mutate(statut_espece_lrr_2025 = case_when(
    is.nan(rate_change) ~ "NaN",
    rate_change <= -80 ~ "CR",
    (rate_change <= -50 & rate_change > -80) ~ "EN",
    (rate_change <= -30 & rate_change > -50) ~ "VU",
    (rate_change <= -20 & rate_change > -30) ~ "NT",
...
glmm_indicateur_critere_A2b_tab <- statut_pourcentage_evo %>%
  select(esp_code_alternatif, 
         rate_change, 
         statut_espece_lrr_2025) %>%
  unique()
```

```{r}
#Représentation graphique du tableau 
glmm_indicateur_critere_A2b_tab %>%
  DT::datatable(rownames = FALSE)

```

## Liste rouge préliminaire 

```{r}
load(file = "../processed_data/liste_especes_lrr.rda")

table_liste_bfc_sp_cor <- table_liste_bfc_sp_cor %>% 
  janitor::clean_names() 
```

```{r}
#Selection des nom vernaculaire : 1 seul
liste_nom_esp <- readxl::read_xlsx("../processed_data/taille_de_maturite_synthese.xlsx",sheet = 1) %>% janitor::clean_names() %>% 
  select(nom_vernaculaire, code_espece)
```


```{r}
table_liste_bfc <- table_liste_bfc_sp_cor %>% 
  left_join(liste_nom_esp, by = "code_espece") %>% 
  select(ordre, 
         nom_scientifique, 
         nom_vernaculaire, 
         code_espece, 
...
```{r}
liste_rouge_prelim <- table_liste_bfc %>% 
  rename(esp_code_alternatif =code_espece) %>% 
  left_join(glmm_indicateur_critere_A2b_tab, by = "esp_code_alternatif") %>% 
  mutate(statut_espece_lrr_2025 = if_else(statut == "allochtone", " ", statut_espece_lrr_2025), 
    statut_espece_lrr_2025 = if_else(is.na(statut_espece_lrr_2025), "NA",statut_espece_lrr_2025), 
    statut_espece_lrr_2025 = factor(statut_espece_lrr_2025, levels = c("CR", "EN", "VU","NT", "LC", "DD", "NA", " "))) %>% 
  arrange(statut_espece_lrr_2025)
   
# Définir les couleurs pour chaque zone
color_EX <- "#000000" # R 0 / V 0 / B 0
color_EW <- "#3D1951" # R 61 / V 25 / B 81
color_RE <- "#5A1A63" # R 90 / V 26 / B 99
color_CR <- "#D3001B" # R 211 / V 0 / B 27
color_EN <- "#FBBF00" # R 251 / V 191 / B 0
color_VU <- "#FFED00" # R 255 / V 237 / B 0
color_NT <- "#FBF2CA" # R 251 / V 242 / B 202
color_LC <- "#78B74A" # R 120 / V 183 / B 74
color_DD <- "#D3D4D5" # R 211 / V 212 / B 213
color_NA <- "#D3D4D5" # R 211 / V 212 / B 213

col_1_7 <- c('Ordre' ,'Nom scientifique','Nom vernaculaire','Code espèce' , 'Statut' , "Taux d'évolution" , 'LRR BFC 2026')

liste_rouge_prelim_brouillon <- liste_rouge_prelim %>%
  select(
    ordre,
    nom_scientifique,
    nom_vernaculaire,
  ...
  rename(
    ...
    'Statut' = statut,
    "Taux d'évolution" = rate_change,
    'LRR BFC 2026' = statut_espece_lrr_2025,
    'LRR France 2019' = lrr_france_2019
  ) %>% 
flextable::flextable() %>%
  flextable::autofit() %>%
  ...
  flextable::bg(i = which(liste_rouge_prelim$statut_espece_lrr_2025 == "CR"),
                bg = color_CR) %>%
  flextable::bg(i = which(liste_rouge_prelim$statut_espece_lrr_2025 == "EN"),
                bg = color_EN) %>%
  flextable::bg(i = which(liste_rouge_prelim$statut_espece_lrr_2025 == "VU"),
                bg = color_VU) %>%
  flextable::bg(i = which(liste_rouge_prelim$statut_espece_lrr_2025 == "NT"),
                bg = color_NT) %>%
  flextable::bg(i = which(liste_rouge_prelim$statut_espece_lrr_2025 == "LC"),
                bg = color_LC) %>%
  flextable::bg(i = which(liste_rouge_prelim$statut_espece_lrr_2025 == "DD"),
                bg = color_DD) %>%
  flextable::bg(i = which(liste_rouge_prelim$statut_espece_lrr_2025 == "NA"),
                bg = color_NA) %>%
  flextable::align(align = "center", part = "all") %>%
  flextable::italic(j = "Nom scientifique", italic = TRUE, part = "body") %>% 
  flextable::font(fontname = "calibri", part = "all") %>% 
  # flextable::bg(part = "all", bg = "white") %>%
  flextable::bg(j = 'LRR France 2019',i = which(liste_rouge_prelim$lrr_france_2019 == "CR"),
                bg = color_CR) %>%
  flextable::bg(j = 'LRR France 2019',i = which(liste_rouge_prelim$lrr_france_2019 == "EN"),
                bg = color_EN) %>%
  flextable::bg(j = 'LRR France 2019',i = which(liste_rouge_prelim$lrr_france_2019 == "VU"),
                bg = color_VU) %>%
  flextable::bg(j = 'LRR France 2019',i = which(liste_rouge_prelim$lrr_france_2019 == "NT"),
                bg = color_NT) %>%
  flextable::bg(j = 'LRR France 2019',i = which(liste_rouge_prelim$lrr_france_2019 == "LC"),
                bg = color_LC) %>%
  flextable::bg(j = 'LRR France 2019',i = which(liste_rouge_prelim$lrr_france_2019 == "DD"),
                bg = color_DD) %>%
  flextable::bg(j = 'LRR France 2019',i = which(liste_rouge_prelim$lrr_france_2019 == "NA"),
                bg = color_NA) %>% 
 flextable::fontsize(size = 16, part = "all") %>% 
    flextable::padding(padding = 5, part = "all") 


liste_rouge_prelim_brouillon
liste_rouge_prelim_brouillon <- flextable::save_as_image(liste_rouge_prelim_brouillon, "../output/lrr_bfc_2026.png")

```

```{r}
liste_rouge_prelim_brouillon <- flextable::save_as_image(liste_rouge_prelim_brouillon, "../output/liste_rouge_prelim.png")
```



## Pour mon rapport, je ne sélectionne que les résultats qui sont significatifs avec les modèles GLMM :

```{r}
glmm_indicateur_critere_A2b_tab_sig <- glmm_indicateur_critere_A2b_tab %>%
  filter(esp_code_alternatif %in% mes_especes)
```
...
# glmm_indicateur_critere_A2b_tab_sig <- glmm_indicateur_critere_A2b_tab_sig %>% 
...
glmm_indicateur_critere_A2b_tab_sig <- glmm_indicateur_critere_A2b_tab_sig %>%
  mutate(pourcentage_evo_cut = ifelse(rate_change > cutoff_value, cutoff_value, rate_change))

# Ajouter une colonne pour indiquer si une valeur a été coupée
glmm_indicateur_critere_A2b_tab_sig <- glmm_indicateur_critere_A2b_tab_sig %>%
  mutate(is_cut = rate_change > cutoff_value)
```


 Les espèces non significatives : "VAI", "TOX", "OBR", "HOT", "EPI", "EPT", "CHA"

```{r, fig.height = 8, fig.width = 14}
# Définir les espèces avec contour plein
species_full_border <- c("ABL", "ANG", "BAF", "BLN", "BRB", "BRE", "BRO", "CCO", "CHE", "GAR", "GOU", "LOF", "LOR", "LOT", "LPP", "PER", "ROT", "SPI", "TAN", "TRF", "VAN")
...
glmm_indicateur_critere_A2b_graph <- glmm_indicateur_critere_A2b_tab_sig %>%
  ggplot(aes(x = reorder(esp_code_alternatif, rate_change), 
             y = pourcentage_evo_cut, 
             fill = factor(sign(rate_change)))) +
  geom_rect(aes(ymin = -Inf, ymax = -80, xmin = -Inf, xmax = Inf), fill = color_CR, alpha = 0.2) +
  geom_bar(aes(linetype = border_type), stat = "identity", color = "black", width = 0.8, alpha = glmm_indicateur_critere_A2b_tab_sig$bar_alpha) +
  ...
  geom_segment(data = glmm_indicateur_critere_A2b_tab_sig %>% 
                 filter(is_cut),
               aes(x = reorder(esp_code_alternatif, rate_change), 
                   xend = reorder(esp_code_alternatif, rate_change),
                   y = pourcentage_evo_cut, 
                   yend = pourcentage_evo_cut + 10),
               linetype = "dotted", color = "black", size = 1.5) +
  geom_text(aes(label = esp_code_alternatif), color = "black", fontface = "bold",vjust = -0.4, size = 4)

print(glmm_indicateur_critere_A2b_graph)
```
