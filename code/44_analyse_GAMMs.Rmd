...
esp_ope_selection_cha <- esp_ope_selection %>%
  ...
  mef_ajouter_type_protocole() %>%
  left_join(ope_selection_test, by = c("ope_id", "annee")) %>%
  rename(valeur = lop_effectif) %>%
  left_join(coord_sta_id, by = "pop_id") %>%
  # left_join(sta_id_ope_selection, by = c("sta_id", "pop_id")) %>%
  mutate(
    ...
    sta_id = as.factor(sta_id)) # creation d'une colonne pour la période (ex : 2007-2008)
  # mutate(periode = {
  #   debut <- floor((annee - annee_debut_periode) / 2) * 2 + annee_debut_periode
  #   paste0(debut)
  # },
  # periode = as.numeric(periode)) %>%
  # rename(annee_1 = annee,
  #        annee = periode)
```
...
```{r}
ggplot(esp_ope_selection_cha, aes(x, y, color = valeur)) + geom_point()
```
```{r}
ggplot(esp_ope_selection_cha, aes(valeur)) + geom_histogram(bins = 30)
```
```{r}
ggplot(esp_ope_selection_cha, aes(log(valeur))) + geom_histogram(bins = 30)
...
# model_3.4 <- glmer(
#   valeur ~ annee + ope_surface_calculee + pro_libelle + julian + (1 |
#                                                                     sta_id),
#   data = esp_ope_selection_cha,
#   family = poisson(link = "log")
# )
...
```{r}
plotres_3 <- plotresid(model_3)
plotres_3.1 <- plotresid(model_3.1)
plotres_3.2 <- plotresid(model_3.2)
# plotres_3.4 <- 
```
...
overdisp.glmer(model_3.1)

overdisp.glmer(model_3.2)
```
