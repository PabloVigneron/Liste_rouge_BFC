...
```{r}
# obr_data <- ope_indicateur %>%
#   filter(esp_code_alternatif == "OBR", indicateur == "effectif_total") 
# 
# nb_station_negative_obr <- obr_data %>%
#   filter(valeur == 0) %>%
#   group_by(annee) %>%
#   summarise(n_total = n_distinct(pop_id)) %>%
#   ungroup()
# 
# nb_station_positive <- obr_data %>%
#   filter(valeur > 0) %>%
#   group_by(annee) %>%
#   summarise(n_total = n_distinct(pop_id)) %>%
#   ungroup() 
...
#   
#   
# lot_data <- ope_indicateur %>%
#   filter(esp_code_alternatif == "LOT", indicateur == "effectif_total") 
# 
# nb_station_negative <- lot_data %>%
#   filter(valeur == 0) %>%
#   group_by(annee) %>%
#   summarise(n_total = n_distinct(pop_id)) %>%
#   ungroup()
# 
# nb_station_positive <- lot_data %>%
#   filter(valeur > 0) %>%
#   group_by(annee) %>%
#   summarise(n_total = n_distinct(pop_id)) %>%
#   ungroup() 
