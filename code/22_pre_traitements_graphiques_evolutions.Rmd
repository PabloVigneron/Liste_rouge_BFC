...
total_valeur_esp_abs <- ope_effectif_glm %>%
  group_by(annee, esp_code_alternatif, indicateur, ope_surface_calculee) %>%
  distinct(ope_surface_calculee) %>% 
  summarise(effectif_total = sum(valeur, na.rm = T), 
            surface_totale = ope_surface_calculee,
            densite_moyenne = mean (effectif_total/surface_totale), .groups = "drop") %>% 
select(esp_code_alternatif, annee, effectif_total,densite_moyenne, surface_totale,  indicateur)


effectif_total_esp_abs <- ggplot(total_valeur_esp_abs, aes(x = annee, y = densite_moyenne)) +
  facet_wrap2(
  ...
  geom_smooth(method = "loess", se = T,color = "darkred") +
  theme(
