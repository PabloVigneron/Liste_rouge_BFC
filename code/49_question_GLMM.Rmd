...
# Exemple d'analyse avec la Truite Fario (EPI)
...
```{r}
plotResiduals(simulateResiduals(fittedModel = mod_glmm_qbn, n = 1000), quantreg = T)

```

```{r}
DHARMa::testZeroInflation(mod_glmm_qbn)

```
```{r}
DHARMa::testDispersion(mod_zi_1.3)
DHARMa::testDispersion(mod_glmm_qbn, plot = F)

```
```{r}
DHARMa::testOverdispersion(mod_zi_1.3)
DHARMa::testOverdispersion(mod_glmm_qbn, plot = F)
...
```
```{r}
DHARMa::plotResiduals(mod_zi_1.3)
plotResiduals(simulateResiduals(fittedModel = mod_zi_1.3, n = 1000), quantreg = T)
```
